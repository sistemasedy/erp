...
editings.addEventListener('click', e => {
  if (e.target.classList.contains('btn-success')) {
	    ...
	    templateEditWarehouse.querySelectorAll('input')[0].value = inv.id
	    templateEditWarehouse.querySelectorAll('input')[1].value = inv.address
	    templateEditWarehouse.querySelectorAll('input')[2].value = inv.name
	    const clone = templateEditWarehouse.cloneNode(true)
	    fragment.appendChild(clone)
	    showWarehouse.appendChild(fragment)
  }
  ...
  if (e.target.classList.contains('btn-primary')) {
	    id = showWarehouse.querySelectorAll('input')[0].value
	    address = showWarehouse.querySelectorAll('input')[1].value
	    name = showWarehouse.querySelectorAll('input')[2].value
	    //console.log(id)
	    let values = {
	    	address: address,
	    	name: name
	    }
	    let index = listWarehouse.findIndex((warehouse) => warehouse.id == id)
	    //console.log(index)
	    Object.assign(listWarehouse[index], values)
	    localStorage.setItem('Warehouse', JSON.stringify(listWarehouse))
...
class WarehouseView {
	...
	createRow(warehouse) {
	    ...
	    Object.values(warehouse).forEach(item =>{
	        ...
	        this.templateWarehouse.querySelectorAll('td')[1].textContent = item.address
	        this.templateWarehouse.querySelectorAll('td')[2].textContent = item.pathner
	        this.templateWarehouse.querySelectorAll('td')[3].textContent = item.name
