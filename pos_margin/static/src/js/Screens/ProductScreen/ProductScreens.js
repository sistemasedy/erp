    ...
    const ProductScreens = ProductScreen =>
        class extends ProductScreen {
            constructor() {
                ...
                var butons = document.getElementsByClassName("mode-button");
                var dat = document.querySelectorAll("mode-button")
                //butons.innerText = 'Monto'
                console.log(butons.HTMLCollection)
                console.log(dat)
                console.log(dat[2])
                useListener('click-superate', this.targetaSuperate);
                ...
                this.hotkey_handler = function(event){
                    if(!$($(document).find("div.payment-screen.screen")[0]).hasClass('oe_hidden')){
                        if (event.which === 119) {
                            $($(document).find("div.button.paymentmethod")[0]).trigger("click");  
                        }
                    }
                    if(!$($(document).find("div.payment-screen.screen")[0]).hasClass('oe_hidden')){
                        if (event.which === 115) {                            
                    ...
                    if(!$($(document).find("div.payment-screen.screen")[0]).hasClass('oe_hidden')){
                        if (event.which === 108) {
                            $($(document).find("#clear")).trigger("click");
                        ...
                        if (event.which === 119) {
                            $($(document).find("#direct2")).trigger("click");
                        }
            ...
            targetaSuperate() {
                ...
                var valSearch = $($(document).find("#superate-search"))[0].value;
                $($(document).find("#resultado"))[0].value = valSearch-total;
                const dif = valSearch - total
                console.log("dif", dif)
                balance.push({bal: dif})
                console.log(balance)
               ...
               if (dif === 0) {
                    ...
                    try{
                         ...
                         this.currentOrder.add_paymentline(this.payment_methods_from_config[1]);
                         this.validateOrder(false)
                         //$($(document).find("#superate-search"))[0].value = '';
                         //$($(document).find("#resultado"))[0].value = '';
            ...
            _setValue(val) {
                if (this.currentOrder.get_selected_orderline()) {
                    ...
                    if (this.state.numpadMode === 'quantity') {
                        ...
                        if (valor > val) {
                            for (var i = val; i != valor; i++) {
                                cantidad = ((val/precio)-.01)
                                this.currentOrder.get_selected_orderline().set_quantity(cantidad);
                            }
            ...
            async _onClickPay() {
                ...
                //this.clear_button_fundir()
            }

            clear_button_fundir(){
                this.currentOrder.add_paymentline(this.payment_methods_from_config[0]);
                //console.log($($(document).find("#selec"))).innerhtml;
                var data = this.env.pos.get_order() //.paymentlines()
                console.log("data", data.get_paymentlines())
