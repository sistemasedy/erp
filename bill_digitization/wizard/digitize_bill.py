...
class DigitizeBill(models.TransientModel):
    ...
    def create_record(self, text):
        ...
        # Lista de patrones para entrenar un modelo de extracción de datos de facturas de proveedores en PDF

        # Patrón para nombre, cantidad, precio y monto en una línea
        newline_name_qty_price_amount = r'\n([A-Za-z ]+) (\d+) (\d+\.\d{2}) (\d+\.\d{2})'

        # Patrón para nombre, cantidad y monto
        name_qty_amount = r'\n([A-Za-z ]+) (\d+) (\d+\.\d{2})'

        # Patrón para cantidad, nombre y monto
        qty_name_amount = r'\n(\d+) ([A-Za-z ]+) (\d+\.\d{2})'

        # Patrón para nombre, cantidad, precio y monto con espacios opcionales
        name_qty_price_amount = r'\s*([A-Za-z() \d]+) (\d+) (\d+) (\d+)'

        # Patrón para nombre, precio, cantidad y monto con símbolo de dólar
        name_price_qty_amount_with_dollar = r'\n(\$?\w+(?: \w+)*) (\$[\d.]+) (\d+) (\$[\d.]+)'

        # Patrón para nombre, precio, cantidad y monto
        name_price_qty_amount = r'\n(\$?\w+(?: \w+)*) (\[\d.]+) (\d+) (\[\d.]+)'

        # Patrón para nombre y monto
        name_amount = r'\n(\$?\w+(?: \w+)*) (\d+\.\d{2})'

        # Patrón para nombre, cantidad, precio y monto en formato dólar
        name_qty_price_dollar_amount = r"([\w\s]+)\s+(\d+)\s+(\d+)\s+\$(\d+)"

        # Patrón para doble línea de nombre
        double_line_name = r'([\w\s]+)\s([\w\s]+)\s*([\w\s]+)\s*(\d+)\s*(\$\d+\.\d{2})\s*(\d+\.\d{2})'

        # Patrón para código y nombre
        code_name = r'Description\n([\s\S]*?)(?=\n\n)'

        # Patrón para cantidad y precio
        quantity_price_pattern = r'Quantity Unit Price Taxes\n([\s\S]*?)(?=\n\n)'

        # Patrón para cantidad, precio y monto
        quantity_price_amount_pattern = r'Quantity Unit Price Taxes Amount\n((?:\d+\.\d+\s+\d+\.\d+\s+\d+\.\d+% \$\s+\d+\.\d+\n)+)'

        # Patrón alternativo para cantidad, precio y monto
        quantity_price_amount_pattern2 = r'Quantity Unit Price Taxes Amount([\s\S]*?)(?:Untaxed Amount|Tax|Total|$)'

        # Patrón para factura de proveedor
        vendor_bill_pattern = r'\[[A-Z0-9-]+\] (.+?) (\d+\.\d+) (\d+\.\d+) (\d+\.\d+%?) \$ (\d+\.\d+)'

        # Patrón para nombre con línea extra
        name_with_extra_line = r'\[([A-Z0-9-]+)\] (.+)'

        # Patrón para fechas en formato dd/mm/yyyy
        date_pattern_ddmmyyyy = r'\d{2}/\d{2}/\d{4}'

        # Patrón para coincidencias de descripciones, cantidades y valores en una línea
        description_qty_value_pattern = r'\n([A-Za-z ]+ \d+(?:,?\d{3})*(?:\.\d{2})?) (\d+(?:,?\d{3})*(?:\.\d{2})?)'

        # Patrón para coincidencias de descripciones y precios para mano de obra y otros costos
        description_price_pattern = r'\n([A-Za-z ]+) (\d{1,3}(?:\.\d{3})*(?:,\d{2})?)'

        # Patrón para coincidir con el total de materiales
        total_materials_pattern = r'\nTotal de materiales (\d{1,3}(?:\.\d{3})*(?:,\d{2})?)'

        # Patrón para coincidir con el total de mano de obra y otros
        total_labor_pattern = r'\nMano de obra y otros (\d{1,3}(?:\.\d{3})*(?:,\d{2})?)'

        # Patrón para presupuestos con múltiples actividades
        project_activity_pattern = r'\n([A-Za-z ]+: [\s\S]+?(?=\n[A-Za-z ]+: |$))'

        # Patrón para coincidir con el número de factura
        bill_no_pattern = r'Bill\sNo\.: (\d{4})'

        # Patrón para coincidir con el año en el formato de la fecha
        year_pattern = re.compile(r'\d{2}')
